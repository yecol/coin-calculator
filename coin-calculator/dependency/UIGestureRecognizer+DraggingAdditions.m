...
- (void)dragViewWithinView:(UIView*)view
            evaluateViewsForOverlap:(NSArray*)views
    containedByOverlappingViewBlock:
        (void (^)(UIView* overlappingView))containsBlock
                         completion:(void (^)(UIView* overlappingView))
                         completionBlock
{
...
- (void)dragViewWithinView:(UIView*)view
            evaluateRectsForOverlap:(NSArray*)rects
    containedByOverlappingRectBlock:
        (void (^)(NSUInteger overlappingIndex))containsBlock
                         completion:(void (^)(NSUInteger overlappingIndex))
                         completionBlock
{
...
- (void)dragLayer:(CALayer*)layer
                         withinView:(UIView*)view
           evaluateRectsForOverlaps:(NSArray*)rects
    containedByOverlappingRectBlock:
        (void (^)(NSUInteger overlappingIndex))containsBlock
                         completion:(void (^)(NSUInteger overlappingIndex))
                         completionBlock
{
    ...
    translatedPoint =
        [[[self view] superview] convertPoint:location
                                     fromView:view];

    CGRect layerRect = [layer frame];
    layerRect = [view convertRect:layerRect
    ...
    case UIGestureRecognizerStateChanged: {
        ...
        // add shadow
        [layer setShadowOffset:CGSizeMake(5, 5)];
        [layer setShadowRadius:5.0];
        [layer setShadowColor:[UIColor blackColor].CGColor];
        [layer setShadowOpacity:0.4];

        NSUInteger containingIndex =
            [UIView indexOfRectContainingRect:layerRect
                                evaluateRects:rects];
...
{
    objc_setAssociatedObject(self, RectValuesKey, rectValues,
...
- (void)setStartPoint:(CGPoint)startPoint
{
    ...
    // If startPoint is CGPointZero, pointValue should be nil so that previous
    // value is released.
    if (CGPointEqualToPoint(startPoint, CGPointZero)) {
        pointValue = nil;
    }

    objc_setAssociatedObject(self, StartPointKey, pointValue,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
